...
async function train(model, data) {
    ...
    const [trainXs, trainYs] = tf.tidy(() => {
        const tr = data.nextTrainBatch(TRAIN_DATA_SIZE);
        return [
            tr.xs.reshape([TRAIN_DATA_SIZE, 28, 28, 1]),
            tr.labels
        ];
    });
    ...
    const [testXs, testYs] = tf.tidy(() => {
        const te = data.nextTrainBatch(TEST_DATA_SIZE);
        return [
            te.xs.reshape([TEST_DATA_SIZE, 28, 28, 1]),
            te.labels
        ];
    });
